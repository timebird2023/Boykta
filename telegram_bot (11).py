...
TELEGRAM_BOT_TOKEN = os.getenv("7848268331:AAFL061-98fzllsZlbpYxLO0otYxGT1-TW4")
if not TELEGRAM_BOT_TOKEN:
    logger.error("❌ TELEGRAM_BOT_TOKEN غير موجود في المتغيرات البيئية!")
    logger.error("يرجى إضافة TELEGRAM_BOT_TOKEN في Secrets")
...
def check_rate_limit(user_id: int) -> tuple[bool, float]:
    """فحص حد المعدل للمستخدم لمنع الإرسال المتكرر"""
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("edit:"):
        ...
        try:
            # جرب GPT-Imager أولاً (الأحدث والأفضل)
            logger.info("🎨 محاولة التحرير بـ GPT-Imager...")
            ...
            # إذا فشل، جرب Nano Banana
            logger.warning("⚠️ GPT-Imager فشل، محاولة Nano Banana...")
            await query.message.reply_text("🔄 جاري المحاولة بنموذج آخر...")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('waiting_for') == 'broadcast_message' and user.id == ADMIN_ID:
    ...
    if context.user_data.get('edit_pending'):
        ...
        # جرب GPT-Imager أولاً
        await message.reply_text(f"🎨 جاري تحرير الصورة بـ GPT-Imager...")
        image_url = ai.gpt_imager(edit_query, photo_url)
        ...
        if ai.is_valid_image_url(image_url):
            ...
            # إذا فشل، جرب Nano Banana
            await message.reply_text("🔄 جاري المحاولة بنموذج آخر...")
            image_url = ai.nano_banana(edit_query, [photo_url])
    ...
    if context.user_data.get('edit_pending_multiple'):
        ...
        # تحرير جميع الصور (GPT-Imager أولاً، ثم Nano Banana)
...
async def process_photo_math_solve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # إرسال النص المستخرج كاملاً بدون قيود
        if len(extracted_text) > 4000:
            parts = [extracted_text[i:i+4000] for i in range(0, len(extracted_text), 4000)]
            for i, part in enumerate(parts):
                if i == 0:
                    await query.message.reply_text(f"📝 **النص المستخرج ({i+1}/{len(parts)}):**\n\n{part}")
                else:
                    await query.message.reply_text(f"📝 **({i+1}/{len(parts)}):**\n\n{part}")
        else:
            await query.message.reply_text(f"📝 **النص المستخرج:**\n\n{extracted_text}")
