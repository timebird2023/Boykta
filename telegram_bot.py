...
def main():
    ...
    try:
        ...
        # إضافة Heartbeat باستخدام threading (متوافق مع Render)
        def heartbeat_thread():
            """مهمة Heartbeat لإبقاء البوت نشطاً (باستخدام threading)"""
            while True:
                try:
                    import requests
        ...
        # بدء Heartbeat في thread منفصل
        heartbeat = threading.Thread(target=heartbeat_thread, daemon=True)
        heartbeat.start()
        logger.info("✅ تم تسجيل Heartbeat Thread (كل 60 ثانية)")
